...
public class TabSecondVisit extends Composite {
	...
	AbsolutePanel panel;
	@UiField
	Label r_mri_biopsy_diagnosis_label;
	@UiField
	Label l_mri_biopsy_diagnosis_label;
	@UiField
	TextBox r_mri_biopsy_diagnosis_other;
	@UiField
	TextBox l_mri_biopsy_diagnosis_other;
	@UiField
	TextBox r_usg_biopsy_diagnosis_other;
	@UiField
	TextBox l_usg_biopsy_diagnosis_other;
	...
	void onR_usg_biopsyClick(ClickEvent event) {
		...
		r_usg_biopsy_diagnosis_label.getElement().getStyle()
	...
	void onR_usgClick(ClickEvent event) {
		...
		r_nonmass.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
		r_usg_biopsy.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
	...
	void onL_usg_biopsyClick(ClickEvent event) {
		...
		l_usg_biopsy_diagnosis_label.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
	}
	...
	void onR_mri_biopsyClick(ClickEvent event) {
		...
		r_mri_biopsy_diagnosis_label.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
	}
	...
	void onL_mri_biopsyClick(ClickEvent event) {
		...
		l_mri_biopsy_diagnosis_label.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
	}
	...
	void onL_usgClick(ClickEvent event) {
		...
		l_nonmass.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
		l_usg_biopsy.getElement().getStyle()
				.setColor(!lockStatus ? "#A8A8A8" : "black");
		...
		for (AnswerUI answer : answers) {
			for (Widget w : panel) {
				if (answer.getAnswerValue() != null
						&& w instanceof HasName
						&& ((HasName) w).getName().equals(
